...
import {changeSelectedTickerCode, changeShowSelectTicker} from "../../redux/actions/containers.action";
import {fetchBymaStocksData} from "../../redux/actions/byma.action";
import {tickerList} from '../../constants/tickers.tsx'
...
const SelectTicker = ({
    ...
    const onListItemClick = ticker => () => {
        onChangeSelectedTickerCode(ticker.code);
        onFetchBymaStocksData();
    };
            ...
            <List>
                {tickerList
                    .filter(
                        ticker =>
                            tickerListSearch === "" ||
                            fuzzySearch(tickerListSearch, ticker.name).length > 0
                    )
                    .map(ticker => (
                        <ListItemButton
                            onClick={onListItemClick(ticker)}
                            key={ticker.code}
                        >
                            <ListItemIcon sx={{mr: 1, transform: "scale(1.2)", minWidth: "unset"}}>
                                {ticker.icon}
                            </ListItemIcon>
                            <ListItemText primary={renderTicker(ticker.name)}/>
                        </ListItemButton>
                    ))}
            </List>
        </Dialog>
