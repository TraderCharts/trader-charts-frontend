...
const router = jsonServer.router('src/resources/apis/$npm_package_config_clientName.mockDB.json');
...
function simpleAuth(req, res, next) {
	...
	if (req.headers.authorization) {
		...
		const user_and_password = new Buffer(req.headers.authorization.split(' ')[1], 'base64').toString();

		// for example, get the username
		const user_only = user_and_password.split(':')[0];
